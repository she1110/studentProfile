...
const DownloadCharacterAnalysisReport = (props:any)=> {
  ...
  const getReportList = () => {
    if (props.dispatch){
      props.dispatch({
        //路径：model的namespace+effects函数名
        type: 'Model_CharacterAnalysisModel/getQuestionnaireListEffects',
        payload: {
  ...
  const downloadReport = (status:any, id:any,name:any) => {
    if (!status){
      ...
      if (props.dispatch){
        props.dispatch({
          //路径：model的namespace+effects函数名
          type: 'Model_CharacterAnalysisModel/getReportEffects',
          payload: {
  ...
  const previewReport = (status:any, id:any,name:any) => {
    if (!status){
      ...
      if (props.dispatch) {
        props.dispatch({
          //路径：model的namespace+effects函数名
          type: 'Model_CharacterAnalysisModel/getPreviewEffects',
          payload: {
            currentUrlInfo: currentUrlInfo,
            callback: (url: any) => {
              // window.open(url)
